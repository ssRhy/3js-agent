...
import { HistoryEntry, useSceneStore } from "../../stores/useSceneStore";
...
interface SidebarProps {
  ...
  handleGenerate: (enableModelGeneration?: boolean) => void;
  isLoading: boolean;
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // New state for model generation toggle
  const [enableModelGeneration, setEnableModelGeneration] = useState(false);
  ...
  // 发送3D场景生成消息时添加到聊天记录
  const handleSendMessage = (messageText?: string, imageData?: string) => {
    const textToSend = messageText || prompt;
    const imageToSend = imageData || selectedImage || undefined;

    if (!textToSend.trim() && !imageToSend) return;
    if (isLoading || socketConnectionStatus !== "open") return;

    const userMessage: ChatMessage = {
      id: Date.now().toString(),
      type: "user",
      content: imageToSend
        ? textToSend.trim() || "根据这张图片生成3D场景"
        : textToSend,
      timestamp: new Date(),
    ...
    const loadingMessage: ChatMessage = {
      ...
      type: "assistant",
      content: imageToSend
        ? "Generating 3D scene from image..."
        : enableModelGeneration
        ? "Generating 3D model and scene..."
    ...
    // 如果有图片数据，调用图片场景生成API
    if (imageToSend) {
      handleImageSceneGeneration(
        textToSend.trim() || "根据这张图片生成3D场景",
        imageToSend
      );
    } else {
      // Pass the model generation flag to handleGenerate
      handleGenerate(enableModelGeneration);
    }

    setPrompt("");
    clearSelectedImage();
    setEnableModelGeneration(false); // Reset after sending
    scrollToBottom();
  };
  ...
  const handleModelUpload = async (file: File) => {
    ...
    try {
      ...
      if (result.success) {
        // 直接加载模型到场景
        if (onModelLoad && result.file?.url) {
          onModelLoad(result.file.url);
        }
        ...
        const successMessage: ChatMessage = {
          ...
          }" uploaded and loaded successfully!`,
          timestamp: new Date(),
        };
  ...
  // 处理图片场景生成
  const handleImageSceneGeneration = async (
    prompt: string,
    imageData: string
  ) => {
    try {
      const response = await fetch("/api/agent", {
        ...
        body: JSON.stringify({
          action: "generate-scene-from-image",
          prompt: prompt,
          imageData: imageData,
        }),
      });
      ...
      if (result.directCode) {
        setCode(result.directCode);

        // 保存到历史记录
        const addHistoryEntryFn = useSceneStore.getState().addHistoryEntry;
        if (addHistoryEntryFn) {
          addHistoryEntryFn(result.directCode, undefined, "根据图片生成3D场景");
        }
        ...
        throw new Error(result.error || "No scene code generated");
      }
    } catch (error) {
  ...
  // Handler for toggling model generation
  const handleToggleModelGeneration = () => {
    setEnableModelGeneration(!enableModelGeneration);
  };
  ...
  return (
    <>
        ...
        {currentView === "chat" && (
          <div className="input-area">
            ...
            <div className="message-input-container">
              ...
              <div className="button-group">
                ...
                <button
                  ...
                  title="GLB and GLTF model file upload"
                >
                  {isUploadingModel ? "..." : "Model"}
                </button>

                {/* Model Generation Toggle Button */}
                <button
                  onClick={handleToggleModelGeneration}
                  disabled={isLoading || socketConnectionStatus !== "open"}
                  className={`model-gen-button ${
                    enableModelGeneration ? "active" : ""
                  }`}
                  title="Enable 3D model generation"
                >
                  {enableModelGeneration ? "Model Gen: ON" : "Model Gen: OFF"}
                </button>
            ...
            <div className="input-hint">
              Press Ctrl+Enter to send • Upload images to generate scenes •
              Upload models to load them directly • Be specific for better
              results
      ...
      <style jsx>{`
        ...
        .model-gen-button:hover:not(:disabled) {
        ...
        .model-gen-button.active:hover:not(:disabled) {
          background: #10b981;
        }
        ...
        .model-upload-button:hover:not(:disabled) {
          background: var(--bg-hover);
          border-color: var(--accent-primary);
          color: var(--accent-primary);
        }

        .model-upload-button:disabled {
          opacity: 0.5;
          cursor: not-allowed;
        }
