...
export default function ObjectManipulationControls() {
  ...
  useEffect(() => {
    ...
    console.log(
      `ObjectManipulationControls: Found ${selectableObjects.length} selectable objects in scene`,
      selectableObjects.map((obj) => ({
        name: obj.name,
        type: obj.type,
        isModel: obj.userData?.isModelObject,
        isUploaded: obj.userData?.isUploadedModel,
        isRemote: obj.userData?.isRemoteModel,
      }))
    );
    ...
    const updateSelectableObjects = () => {
      if (dragControlsRef.current) {
        ...
        console.log(
          `ObjectManipulationControls: Updated selectable objects count: ${updatedObjects.length}`,
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        // Log selection info for uploaded models
        if (
          targetObject.userData?.isUploadedModel ||
          targetObject.userData?.isRemoteModel
        ) {
          console.log("Selecting uploaded/remote model:", {
            name: targetObject.name,
            type: targetObject.type,
            isUploaded: targetObject.userData?.isUploadedModel,
            isRemote: targetObject.userData?.isRemoteModel,
            modelId: targetObject.userData?.modelId,
