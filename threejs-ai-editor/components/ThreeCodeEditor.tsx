...
export default function ThreeCodeEditor() {
  ...
  useEffect(() => {
    try {
      ...
      const handleScreenshotRequest = async (data: {
        ...
        console.log(
          `[ThreeCodeEditor] 🎯 CLIENT RECEIVED screenshot request: ${data.requestId} (socket ID: ${socket.id})`
        );
        console.log(`[ThreeCodeEditor] 📊 Request details:`, data);
        ...
        try {
          ...
          console.log(
            `[ThreeCodeEditor] 📸 CLIENT ${socket.id} responding with screenshot for request: ${data.requestId}`
          );
          ...
          console.log(
            `[ThreeCodeEditor] ✅ CLIENT ${socket.id} screenshot sent for request: ${data.requestId}`
          );
        } catch (error) {
          console.error(
            `[ThreeCodeEditor] ❌ CLIENT ${socket.id} screenshot request processing failed:`,
            error
          );
  ...
  const handleGenerate = async (enableModelGeneration: boolean = false) => {
  ...
  // 处理模型加载
  const handleModelLoad = async (url: string) => {
    try {
