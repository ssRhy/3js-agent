...
export function cleanCodeOutput(output: unknown): string {
  ...
  if (
    !codeIndicators.some((indicator) => codeOutput.includes(indicator)) ||
    isExplanatoryText(codeOutput)
  ) {
    // 尝试检测是否是思考过程或解释文本
  ...
  if (!codeOutput.includes("function setup")) {
    const cleanedCode = codeOutput
      .split("\n")
      .filter((line) => !isExplanatoryText(line))
      .join("\n");
