 ...
 * 验证代码响应是否为纯代码
 */
function validateCodeResponse(response: string): boolean {
  // 检查是否包含解释性文本
  ...
  const hasCodeStructure = codeStructurePatterns.some((pattern) =>
    pattern.test(response)
  );

  return hasCodeStructure && !hasExplanatoryText;
}
...
function handleLLMResponseContent(content: unknown): string {
  ...
  // 验证响应是否为纯代码
  if (!validateCodeResponse(processedContent)) {
    console.warn("检测到非代码响应，生成基础场景代码");
...
6. **CRITICAL - Model Ground Alignment**: When loading 3D models, ensure they are properly positioned on the ground:
   - After loading and scaling a model, calculate its bounding box
   - Position the model so its bottom surface touches the ground (y=0)
   - Use: model.position.y = -boundingBox.min.y (NOT modelHeight/2)
   - Never make models float above the ground
7. **Position Calculation Order**: Always scale models FIRST, then recalculate bounding box for positioning
