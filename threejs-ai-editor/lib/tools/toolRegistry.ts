...
import { imageSceneTool } from "./imageSceneTool";
...
export class ToolRegistry {
  ...
  private initializeTools(): void {
    ...
    this.registerTool(
      imageSceneTool.name,
      imageSceneTool as unknown as Tool,
      ToolCategory.MODEL
    );
   ...
   * 获取所有工具，支持排除特定工具
   * @param category 可选的工具类别过滤
   * @param excludeModelGen 是否排除模型生成工具
   * @returns 工具数组
   */
  public getAllToolsWithExclusions(
    category?: ToolCategory,
    excludeModelGen: boolean = false
  ): Tool[] {
    const tools: Tool[] = [];

    this.tools.forEach((tool, name) => {
      // 如果需要排除模型生成工具且当前工具是模型生成工具，则跳过
      if (excludeModelGen && name === "generate_3d_model") {
        console.log(
          `[ToolRegistry] Excluding model generation tool as requested`
        );
        return;
      }
  ...
  public getModelGenerationTools(): Tool[] {
    const modelGenTool = this.tools.get("generate_3d_model");
    return modelGenTool ? [modelGenTool] : [];
  }
